...
def build_field_vocab(field, counter, **kwargs):  # *args表示任何多个无名参数，它是一个tuple；**kwargs表示关键字参数，它是一个 dict
    ...
    specials = list(OrderedDict.fromkeys(
        tok for tok in [field.unk_token, field.pad_token, field.init_token, field.eos_token, Constants.MASK_WORD]
        if tok is not None))
...
def merge_vocabs(vocabs, vocab_size=None, min_frequency=1):
    ...
    return torchtext.vocab.Vocab(merged,
                                 specials=[Constants.UNK_WORD, Constants.PAD_WORD,
                                           Constants.BOS_WORD, Constants.EOS_WORD, Constants.MASK_WORD],
                                 max_size=vocab_size,
...
def build_vocab(train_dataset_files, fields, share_vocab,
    ...
    merged_structure_vocab = merge_vocabs(
        [fields["structure1"].vocab, fields["structure2"].vocab, fields["structure3"].vocab, fields["structure4"].vocab,
         fields["structure5"].vocab],
                    ...
                    with open(structure_corpus3, "r") as structure_file3:
                        with open(structure_corpus4, "r") as structure_file4:
                            with open(structure_corpus5, "r") as structure_file5:
                                ...
                                for s, t, structure1, structure2, structure3, structure4, structure5 in zip(src_file,
                                                                                                            tgt_file,
                                                                                                            structure_file1,
                                                                                                            structure_file2,
                                                                                                            structure_file3,
