...
def make_features(batch, side):
    ...
    """
    assert side in ['src', 'tgt', 'structure1', 'structure2', 'structure3', 'structure4', 'structure5', 'structure6',
                    'structure7', 'structure8', 'noised_tgt']
...
def load_fields(opt, checkpoint):
    ...
    logger.info(
        ' * vocabulary size. source = %d; target = %d; structure1 = %d; structure2 = %d; structure3 = %d; structure4 = %d; structure5 = %d; '
        %
    ...
    def __iter__(self):
        ...
        while self.cur_iter is not None:
            for batch in self.cur_iter:
                tgt_outer, bert_tgt = self._mask_data(batch.tgt.clone().to('cpu'))  # tgt_out and batch
    ...
    def _mask_data(self, batch_sentences):
        ...
        for sent_index in range(batch_sentences.size(1)):
            ...
            num_to_predict = min(20,
                                 max(1, int(round(len(cand_indexes) * 0.20))))
            num_has_predicted = 0
            covered_indexes = set()
        ...
        for ex_values in example_values:
            ...
            for (name, field), val in zip(out_fields, ex_values):
                ...
                if field is not None:
                    setattr(example, name,
                            field.preprocess(val))  # setattr(object, name, values)给对象的属性赋值，若属性不存在，先创建再赋值。
