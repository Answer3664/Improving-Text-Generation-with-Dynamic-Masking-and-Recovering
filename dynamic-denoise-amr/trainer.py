...
def build_trainer(opt, device_id, model, fields,
    ...
    trainer = Trainer(model, train_loss, valid_loss, optim, trunc_size,
                      ...
                      use_masked_tgt=opt.use_masked_tgt,
                      tgt_vocabs=fields['tgt'].vocab,
...
class Trainer(object):
    ...
    def __init__(self, model, train_loss, valid_loss, optim,
                 ...
                 gpu_verbose_level=0, report_manager=None, model_saver=None, use_masked_tgt=False,
                 tgt_vocabs=None, max_predictions_per_seq=20, masked_lm_prob=0.15, alpha=0.1):
        # Basic attributes.
        self.model = model
        ...
        self.use_masked_tgt = use_masked_tgt
            ...
            for i, batch in enumerate(train_iter):
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    if self.n_gpu:
                        ...
                        batch.noised_tgt = batch.noised_tgt.to(device)
                        batch.bert_tgt = batch.bert_tgt.to(device)
                    # if self.use_masked_tgt:
                    ...
                    if self.norm_method == "tokens":
                        ...
                        replace_tokens = batch.bert_tgt.ne(
                            self.train_loss.padding_idx).sum()
                        bert_normalization += replace_tokens
                    else:
                        normalization += batch.batch_size
                        bert_normalization += batch.batch_size
                    accum += 1
                    if accum == self.grad_accum_count:
                        ...
                        self._gradient_accumulation(
                            true_batchs, bert_normalization, normalization, total_stats,
                            report_stats)
    ...
    def validate(self, valid_iter):
        ...
        for batch in valid_iter:
            ...
            outputs, _, attns = self.model(src, tgt, structure1, structure2, structure3, structure4, structure5,
                                        src_lengths)
    ...
    def _gradient_accumulation(self, true_batchs, bert_normalization, normalization, total_stats,
                               report_stats):
        ...
        for batch in true_batchs:
            ...
            tgt_outer = make_features(batch, 'noised_tgt')
            #if self.use_masked_tgt:
            #    tgt_outer, bert_tgt = self._mask_data(batch.tgt.clone())  # tgt_out and batch
            #    batch.__dict__['noised_tgt'] = tgt_outer
            #    batch.__dict__['bert_tgt'] = bert_tgt
                # tgt_outer = self._mask_data(tgt_outer.clone())  # tgt_out and batch
            ...
            for j in range(0, target_size - 1, trunc_size):
                ...
                if self.grad_accum_count == 1:
                    self.model.zero_grad()
                outputs, select_out, attns = self.model(src, tgt, structure1, structure2, structure3, structure4, structure5,
                                            src_lengths)

                # 3. Compute loss in shards for memory efficiency.
                batch_stats = self.train_loss.sharded_compute_loss(
                    batch, outputs, select_out, attns, j,
                    trunc_size, self.shard_size, bert_normalization, normalization, self.alpha)
    ...
    def _mask_data(self, batch_sentences):
        ...
        bert_tgt = torch.full_like(batch_sentences, self.tgt_vocabs.stoi[Constants.PAD_WORD])
        specials = list(range(num_special_tokens))
        masked_token_id = self.tgt_vocabs.stoi[Constants.MASK_WORD]
            ...
            for index in cand_indexes:
                ...
                if rng.random() < 0.8:
                    ...
                    # 10% of the time, replace with random word
                        ...
                        masked_token = rng.randint(num_special_tokens, len(self.tgt_vocabs) - 1) #  a <= x < b
