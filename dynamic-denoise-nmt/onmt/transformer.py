...
from onmt.transformer_decoder import TransformerDecoder, BertDecoder
...
class NMTModel(nn.Module):
    def __init__(self, encoder, decoder, bert_decoder):
    ...
    def forward(self, src, tgt, lengths):
        ...
        dec_out, select_out, attns = self.decoder(tgt)

        self.bert_decoder.init_state(src, memory_bank, select_out)
        bert_out = self.bert_decoder(tgt)
        return dec_out, bert_out, attns
...
def build_decoder(opt, embeddings):
    ...
    """
    return TransformerDecoder(opt.dec_layers, opt.select_layer, opt.dec_rnn_size,
                              opt.heads, opt.transformer_ff,
                              opt.dropout, embeddings)


def build_bert_decoder(opt, embeddings):
    return BertDecoder(opt.bert_dec_layers, opt.dec_rnn_size,
                       opt.heads, opt.transformer_ff,
                       opt.dropout, embeddings)
...
def build_base_model(model_opt, fields, gpu, checkpoint=None):
    ...
    bert_decoder = build_bert_decoder(model_opt, tgt_embeddings.word_padding_idx)

    # Build NMTModel(= encoder + decoder).
    device = torch.device("cuda" if gpu else "cpu")
    model = NMTModel(encoder, decoder, bert_decoder)
    ...
    if model_opt.share_decoder_embeddings:
        ...
        # bert_gen[0].weight = decoder.embeddings.word_lut.weight
    ...
    if checkpoint is not None:
        ...
        # bert_gen.load_state_dict(checkpoint['bert_gen'], strict=False)
    else:
